...
const CreateStory = ({ currentLanguage }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!currentUser) {
      setError(isThaiLanguage ? 'กรุณาเข้าสู่ระบบก่อนสร้างเรื่องราว' : 'Please login to create a story');
      return;
    }
    ...
    try {
      ...
      setError(isThaiLanguage ? 'เกิดข้อผิดพลาดในการสร้างเรื่องราว' : 'Failed to create story');
    } finally {
      setSaving(false);
      setUploading(false);
    }
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#0f0f0f] pt-20">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-4">
            {isThaiLanguage 
              ? 'คุณต้องเข้าสู่ระบบก่อนสร้างเรื่องราว'
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold text-white mb-2">
            {isThaiLanguage ? 'สร้างเรื่องราวใหม่' : 'Create New Story'}
          </h1>
          <p className="text-gray-400">
            {isThaiLanguage 
              ? 'แบ่งปันเรื่องราวของคุณกับผู้อ่านทั่วโลก'
          ...
          <div className="bg-[#1a1a1a] rounded-lg border border-[#2a2a2a] p-6 space-y-6">
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder={isThaiLanguage ? 'เขียนคำอธิบายสั้นๆ เกี่ยวกับเรื่องราว' : 'Write a short description about your story'}
                rows={3}
                required
              />
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-white mb-2">
                {isThaiLanguage ? 'เนื้อหา' : 'Content'} <span className="text-red-500">*</span>
              </label>
              <Textarea
                value={content}
                onChange={(e) => setContent(e.target.value)}
                placeholder={isThaiLanguage ? 'เขียนเรื่องราวของคุณที่นี่...' : 'Write your story here...'}
                rows={20}
                className="bg-[#0f0f0f] border-[#2a2a2a] text-white resize-y"
                required
              />
            </div>
          </div>
            ...
            >
              {uploading ? (
                ...
                <>
                  ...
                  {isThaiLanguage ? 'เผยแพร่เรื่องราว' : 'Publish Story'}
                </>
              )}
            </Button>
          </div>
