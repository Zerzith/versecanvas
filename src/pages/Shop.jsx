...
export default function Shop() {
  ...
  const handlePurchase = async (product) => {
    ...
    try {
      // หักเครดิตจากผู้ซื้อ
      await deductCredits(product.price, `ซื้อ ${product.title}`);

      // โอนเครดิตให้ผู้ขาย (ในระบบจริงต้องมีการหักค่าธรรมเนียม)
      // await transferCredits(product.sellerId, product.price * 0.9, `ขาย ${product.title}`);

      alert(`ซื้อสินค้าสำเร็จ! คุณได้ซื้อ "${product.title}" แล้ว`);
